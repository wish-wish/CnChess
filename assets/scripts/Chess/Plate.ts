    ...
    initPos()
    {
        ...
        {            
            for(let j=1;j<this.blackpos[i].length;j++)
            {
                ...
                this.allpos.set(this.blackpos[i][j],Utils.blackchs[this.blackpos[i][0]]);
                this.allpos.set(this.redpos[i][j],Utils.redchs[this.redpos[i][0]]);                
            }
        }
        Utils.xChessPos=[];
        for(let i=0;i<this.xPlusPot.length;i++)
        {
            Utils.xChessPos.push(this.xPlusPot[i]);
        }
        for(let i=0;i<this.xPlusPot.length;i++)
        {
            Utils.xChessPos.push(this.xPoints.length-this.xPlusPot[i]-1);
        }
    }
    start() {

  ...
  return {
    ...
    execute: function () {
      ...
      _export("Hrdaya", Hrdaya = (_dec = ccclass('Hrdaya'), _dec(_class = (_class2 = class Hrdaya extends Component {
        ...
        onLoad() {
          var self = this;
          var g = self.node.getComponent(Graphics);
          self.g = g;
        }
        ...
        reCurve() {
          ...
          if (self.mode == 0) {
            ...
            var rect = self.node.getComponent(UITransform);
            var scalew = self.width / rect.width;
            var scaleh = self.height / rect.height;

            for (var _i = 0; _i < self.arr.length; _i++) {
              self.arr[_i][0] /= scalew;
              self.arr[_i][1] /= scaleh;
            }
            ...
            var g_fan = self.node.getComponentInChildren(Graphics);
        ...
        drawFragile(cb) {
          ...
          var g = self.g;
          var node = self.node.getChildByName("Node");

          if (self.isAnimDraw) {
            var g_fan = node.getComponent(Graphics);
        ...
        hitRect(pt) {
          ...
          var n = self.node;
          var rect = [];
          var offset = new Vec2(n.position.x, n.position.y);
          var vs = view.getCanvasSize();
          rect.push(new Vec2(-self.width / 2 + offset.x + vs.width / 2, -self.height / 2 + offset.y + vs.height / 2));
          rect.push(new Vec2(self.width / 2 + offset.x + vs.width / 2, self.height / 2 + offset.y + vs.height / 2));
          var isin = self.ptInRect(pt, rect);
          return isin;
        }
        ...
        hitRect1(pt) {
          ...
          var rect = [];
          var offset = new Vec2(n.position.x, n.position.y);
          var scale = new Vec3(n.scale.x, n.scale.y, n.scale.z);
          var point = [];
          screen.windowSize;
          ...
          var isin = (_crd && PointInRect === void 0 ? (_reportPossibleCrUseOfPointInRect({
            error: Error()
        ...
        hitRect2(pt) {
          ...
          var rect = [];
          var offset = new Vec2(n.position.x, n.position.y);
          var scale = new Vec3(n.scale.x * 1.13, n.scale.y * 1.13, n.scale.z * 1.13);
          var cs = view.getCanvasSize();
          var ds = view.getDesignResolutionSize();
          var fs = view.getFrameSize();
          var x = pt[0] / cs.width * ds.width;
          var y = pt[1] / cs.height * ds.height;
          var fh = ds.width / fs.width * fs.height;
          var scaleh = fh / ds.height;
          var pt1 = [x - ds.width / 2, (y - ds.height / 2) * scaleh];
        ...
        drawOutline(scale) {
          ...
          var box = self.node.getChildByName("box"); // if(!box)
          // {
          //     box=new Node();
          //     box.name="box"
          ...
          var g_box = box.getComponent(Graphics);
        ...
        Hrdaya(len, pos, count) {
          var self = this;
          var num = Math.sqrt(len);
          var x = pos % num;
          var y = Math.trunc(pos / num);
          var arc = 0;
          var big = 16;
          ...
          var radius = Math.min(count / Math.PI / 4, 1.4);
          var start = 0;
          var toset = 0;
          var half = Math.trunc(Math.ceil(count / 2));
          var arr = [];
          ...
          for (var i = 0; i < count; i++) {
            ...
            var xp = radius * 16 * Math.pow(Math.sin(start + arc * toset), 3);
            var yp = radius * (13 * Math.cos(start + arc * toset) - 5 * Math.cos(start + 2 * arc * toset) - 2 * Math.cos(start + 3 * arc * toset) - Math.cos(start + 4 * arc * toset));
            var offsety = 1.7;
...
//# sourceMappingURL=ba9dd245135ec5a448cf4ddb61a4fb04db43dac7.js.map
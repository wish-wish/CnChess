  ...
  return {
    ...
    execute: function () {
      ...
      _export("Head", Head = (_dec = ccclass('Head'), _dec(_class = class Head extends Component {
        ...
        start() {
          var pathjson = [["m 62.4,27.3 14.7,7.5 21,4.5 h 8.1 v 42.3 l -0.9,19.5 -1.8,2.7 -14.1,5.4 -14.1,15.9 v 23.4 l 10.2,13.8 H 66.3 l -16.8,-3.6 -15.3,-3.3 -12.6,-17.1 -6.9,-26.1 3,-23.7 6,-15.6 L 32.1,51.3 44.7,39", "path24"]]; //    [
          ...
          this.node.on(Node.EventType.TOUCH_START, touch => {
            var pt = this.getTouchPoint(this.node, touch);
        ...
        getTouchPoint(node, touch) {
          ...
          var trans = self.node.getComponent(UITransform);
          var size = trans.contentSize;
          var pt = new Vec2(touchLoc.x / view.getScaleX() - size.width / 2 - pos.x, touchLoc.y / view.getScaleY() - size.height / 2 - pos.y);
          return pt;
        }
        ...
        parsePolygons(s) {
          ...
          for (var i = 0; i < s.length; i++) {
            var pts = s[i][0].split(' ');
            var polygon = new (_crd && Polygon === void 0 ? (_reportPossibleCrUseOfPolygon({
              error: Error()
            }), Polygon) : Polygon)();
            var cmds = "MHVLmhvl";
            var p = null;
            ...
            while (j < pts.length) {
              ...
              if (p.act == "M") {
                var v = new Vec2(Number.parseFloat(xy[0]), Number.parseFloat(xy[1]));
                last = v;
                polygon.poly.push(v);
                p.pt = v;
                j = j + 1;
              } else if (p.act == "H") {
                var _v = new Vec2(Number.parseFloat(xy[0]), last.y);

                last = _v;
                polygon.poly.push(_v);
                p.pt = _v;
                j = j + 1;
              } else if (p.act == "V") {
                var _v2 = new Vec2(Number.parseFloat(xy[0]));

                last = _v2;
                polygon.poly.push(_v2);
                p.pt = _v2;
                j = j + 1;
              } else if (p.act == "L") {
                var _v3 = new Vec2(Number.parseFloat(xy[0]), Number.parseFloat(xy[1]));

                last = _v3;
                polygon.poly.push(_v3);
                p.pt = _v3;
                j = j + 1;
              } else if (p.act == "Z") {
                p.pt = null;
              } else if (p.act == "m") {
                var _v4 = new Vec2(last.x + Number.parseFloat(xy[0]), last.y + Number.parseFloat(xy[1])); //let v=new Vec2(Number.parseFloat(xy[0]),Number.parseFloat(xy[1]));


                last = _v4;
                polygon.poly.push(_v4);
                p.pt = _v4;
                j = j + 1;
              } else if (p.act == "h") {
                var _v5 = new Vec2(last.x + Number.parseFloat(xy[0]), last.y);

                last = _v5;
                polygon.poly.push(_v5);
                p.pt = _v5;
                j = j + 1;
              } else if (p.act == "v") {
                var _v6 = new Vec2(last.x, last.y + Number.parseFloat(xy[0]));

                last = _v6;
                polygon.poly.push(_v6);
                p.pt = _v6;
                j = j + 1;
              } else if (p.act == "l") {
                var _v7 = new Vec2(last.x + Number.parseFloat(xy[0]), last.y + Number.parseFloat(xy[0]));

                last = _v7;
                polygon.poly.push(_v7);
                p.pt = _v7;
                j = j + 1;
              } else if (p.act == "z") {
                p.pt = null;
        ...
        draw() {
          ...
          for (var i = 1; i < this.polygons[0].poly.length; i++) {
...
//# sourceMappingURL=145b13a489c79e54d0b60d41a6a80627058d1105.js.map
System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, _decorator, Component, Color, Graphics, UITransform, Size, Vec3, Label, math, instantiate, RichTextComponent, SpriteComponent, Utils, _dec, _dec2, _dec3, _class, _class2, _descriptor, _descriptor2, _crd, ccclass, property, Plate;
    ...
    execute: function () {
      ...
      _export("Plate", Plate = (_dec = ccclass('Plate'), _dec2 = property({
        ...
        start() {
          ...
          (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            ...
            this.allpos.forEach((img, key) => {
              ...
              var sprite = n.getComponentInChildren(SpriteComponent);
              sprite.node.active = true;
              sprite.n.setParent(this.node);
            ...
            for (var _i3 = 0; _i3 < ords.length; _i3++) {
              ...
              if (_i3 > 8) {
                idx = this.xPoints.length - _i3 - 1;
                var pos = new Vec3(this.xPoints[idx].x, this.rect[2].y, 0);

                _n.setPosition(pos);
              } else {
                var _pos = new Vec3(this.xPoints[idx].x, this.rect[0].y, 0);
          ...
          (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            ...
            for (var _i4 = 0; _i4 < ords.length; _i4++) {
              var _n2 = instantiate(prefab);

              var _rtc2 = _n2.getComponentInChildren(RichTextComponent);

              _rtc2.string = ords[_i4];
              _rtc2.fontSize = 30;

              _n2.setParent(this.node);

              var idx = _i4;

              if (_i4 > 8) {
                idx = this.xPoints.length - _i4 - 1;
                var pos = new Vec3(this.xPoints[idx].x, this.rect[2].y, 0);

                _n2.setPosition(pos);
              } else {
                var _pos2 = new Vec3(this.xPoints[idx].x, this.rect[0].y, 0);

                _n2.setPosition(_pos2);
              }
            }
          });
        }
        ...
        draw() {
          ...
          var row = 9,
              col = 8;
          var width = (s.width - border * 2 - edge * 2) / (col + 1);
          var height = (s.height - border * 2 - edge * 2) / (row + 1);
          ...
          for (var _i5 = 0; _i5 < pts.length; _i5++) {
            this.grap.lineTo(pts[_i5].x, pts[_i5].y);
          }

          for (var _i6 = 0; _i6 < col + 1; _i6++) {
            var leftx = pts[0].x + _i6 * width + width / 2 + edge;
            var lefty = pts[0].y + height / 2 + edge;
          ...
          for (var _i7 = 0; _i7 < row + 1; _i7++) {
            var _leftx = pts[0].x + width / 2 + edge;

            var _lefty = pts[0].y + height / 2 + edge;

            this.grap.moveTo(_leftx, _lefty + height * _i7);
            this.grap.lineTo(_leftx + s.width - border * 2 - width - edge * 2, _lefty + height * _i7);
          }

          this.grap.moveTo(this.xPoints[this.XRoad[0]].x, this.xPoints[this.XRoad[0]].y);

          for (var _i8 = 1; _i8 < this.XRoad.length; _i8++) {
            var idx = this.XRoad[_i8];
          ...
          for (var _i9 = 1; _i9 < this.XRoad.length; _i9++) {
            var _idx = this.xPoints.length - this.XRoad[_i9] - 1;
          ...
          for (var _i10 = 0; _i10 < this.xPlusPot.length; _i10++) {
            var p = this.xPoints[this.xPlusPot[_i10]];
            this.drawCorss(p.x, p.y);
            var p1 = this.xPoints[this.xPoints.length - this.xPlusPot[_i10] - 1];
...
//# sourceMappingURL=99981375307b6b2b86e030a46313ea2f56b65c90.js.map